GetSymbols.netease <- function(code, market, from, to, savefile, dfname,
  ...
  if (is.na(dfname) & is.na(savefile)) {
    ...
    if (!is.na(dfname)) {
      temptable$DATE <- as.POSIXct(temptable$DATE)
      temptable <- xts::as.xts(subset(temptable, select = c("TOPEN", "HIGH", "LOW", "TCLOSE", "VOTURNOVER")), 
                               order.by = temptable$DATE, 
                               dateFormat = "POSIXct")
      assign(dfname, temptable, envir=parent.frame())
      dfname <- quantmod::as.quantmod.OHLC(dfname, col.names = c("Open", "High","Low", "Close", "Volume"))
